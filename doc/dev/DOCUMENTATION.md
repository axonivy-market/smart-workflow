# Writing Documentation

House style for `doc/user/`, the published documentation set. Follow it when adding or editing a page, so the set reads as one document rather than a pile of separate pages.

## Structure

**One feature, one file.** Each feature has a single page at the root of `doc/user/` covering everything about it — how to configure it, its variables, its failure modes. A change to a feature should be a single-file edit.

| Location | Holds |
| --- | --- |
| _(root)_ | One page per feature, plus [Getting Started](../user/getting-started.md), [Concepts](../user/concepts.md) and [Troubleshooting](../user/troubleshooting.md) |
| `reference/` | Only what **several** features read — the capability matrix, the variables table, the error codes |

Do not create a second page for one feature. Splitting a feature across files is what makes it stale: the copy someone forgets is always the one still being read.

List every new page in the [documentation index](../../README.md) in the repository root. A page nothing links to does not exist.

## `doc/user/` is for using, `doc/dev/` is for extending

`doc/user/` answers "how do I use this feature?" Anything that answers "how do I add one?" belongs in [Extending Smart Workflow](EXTENDING.md) instead — implementing an interface, SPI registration, project setup, contribution checklists.

A feature page may say that the extension point exists, in a sentence or two, and link to it. It should not explain how to build one.

That keeps the published set free of content its readers cannot act on, and keeps each extension's instructions in one place rather than spread across the feature pages it touches.

## `doc/user/` must stay self-contained

That folder is built on its own by an external doc tool. A relative link that leaves it resolves inside the repository and breaks in the generated site.

- Links **inside** `doc/user/` are relative: `reference/variables.md` from a feature page, `../agent-setup.md` from `reference/`.
- Links to **anything else** — source files, demo processes, `pom.xml`, `doc/dev/` — are absolute:
  `https://github.com/axonivy-market/smart-workflow/blob/master/…`
- Images live in `doc/user/img/` and are never referenced from outside the folder.

Nothing checks this automatically, and an escaping link still works on GitHub — so it is worth a deliberate look whenever you add one.

`doc/dev/` has the opposite rule: it is never published, so relative links out of it (`../user/rag.md`, `../../pom.xml`) are correct and preferred.

## One owner per fact

Before writing a fact down, check whether a reference page already owns it:

| Fact | Owner |
| --- | --- |
| What a provider supports | [Provider Capabilities](../user/reference/capabilities.md) |
| Any `AI.*` variable | [Variables](../user/reference/variables.md) |
| Any error code, and the problems that raise none | [Error Codes](../user/reference/error-codes.md) |
| Agent element field semantics | [Agent Setup](../user/agent-setup.md) |
| Any Java interface you implement to extend Smart Workflow | [Extending Smart Workflow](EXTENDING.md) |

If it has an owner, **link — do not restate**. Duplicated facts drift, and the copy that goes stale is never the one you are reading.

## Page template

```
# <Noun or task>

One paragraph: what this is and when you need it.

## Prerequisites        ← link, never repeat
## <the work>
## Example              ← exactly one, complete
## Common mistakes      ← real traps, not generic cautions
## See also
```

**Common mistakes** is not filler. Several problems do not raise an error, so the section that names them is the highest-value content on most pages. Prefer a real trap you have hit over a generic caution.

## Conventions

**Product name** — "Smart Workflow", two words, always. Never hyphenated, never translated, including in a translated page's H1.

**Spelling** — en-GB word choice (`defence`, `behaviour`, `customise`) with Oxford `-ize` endings (`organization`, `anonymized`). Add genuinely new terms to `.github/cspell-extra-words.txt`; do not add a word to silence an inconsistency you should fix instead.

**Headings** — sentence case: `## Handling a stopped agent`, not `## Handling A Stopped Agent`. Never skip a level; an `##` is always followed by `##` or `###`, never `####`.

**Emoji** — do not use them in headings or when naming a Designer UI group. Referring to a group as **Message** rather than **✉️ Message** keeps the docs correct when the UI icons change. Emoji also corrupt the generated anchor slug: `## 🎭 Mocking` becomes `#-mocking`, not `#mocking`.

**Field values** — a two-column table, not bold prose labels:

```markdown
| Field | Value |
| --- | --- |
| `Map result to` | `in.result` |
```

**Tables** — spaced delimiter rows: `| --- | --- |`, or `| :---: |` to centre.

**Callouts** — `> **Important:**` for something that will bite the reader, `> **Note:**` for an aside. Use them sparingly; a page of callouts has no emphasis at all.

**Code blocks** — tag them with a language where one applies (`yaml`, `java`, `xml`, `json`). Leave the fence bare for plain text such as prompts and log lines; some renderers try to interpret a `text` tag and fail on it.

## Before opening a PR

- [ ] the page is listed in the index in the repository root `README.md`
- [ ] no relative link leaves `doc/user/`, and every link target exists
- [ ] no fact duplicated from a reference page, and nothing about this feature left on another page
- [ ] nothing about implementing or registering an extension — that belongs in [Extending](EXTENDING.md)
- [ ] a **Common mistakes** section, if the topic has any
- [ ] headings sentence-cased, no heading level skipped
- [ ] `README_DE.md` is a translation of an older revision — do not sync it by hand

## See also

- [Documentation index](../../README.md) — the reading order this style serves
- [Extending Smart Workflow](EXTENDING.md) — where contributor instructions live, and the doc updates a new provider requires
