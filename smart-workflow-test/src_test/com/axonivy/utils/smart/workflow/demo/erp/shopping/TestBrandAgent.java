...
class TestBrandAgent {
  ...
  void setup(AppFixture fixture) {
    ...
    MockOpenAI.defineChat(new ShoppingChat()::brandAgentResponse);
  }

  @Test
  void callBrandAgent(BpmClient client) {
    var res = client.start().process(TEST_PROCESS.elementName("testBrandAgent")).execute();
    DemoTestProcessData data = res.data().last();
    assertThat(data.getBrandAgentResult()).isEqualTo("Brand agent processed successfully.");
  }
}
