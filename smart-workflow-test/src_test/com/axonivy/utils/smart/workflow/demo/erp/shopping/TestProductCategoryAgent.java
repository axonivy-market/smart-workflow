...
@RestResourceTest
class TestProductCategoryAgent {
  ...
  void setup(AppFixture fixture) {
    ...
    MockOpenAI.defineChat(new ShoppingChat()::categoryAgentResponse);
  }

  @Test
  void callProductCategoryAgent(BpmClient client) {
    var res = client.start().process(TEST_PROCESS.elementName("testProductCategoryAgent")).execute();
    DemoTestProcessData data = res.data().last();
    assertThat(data.getCategoryAgentResult()).isEqualTo("Category agent processed successfully.");
  }
}
