...
class TestTranslationAgent {
  ...
  void setup(AppFixture fixture) {
    ...
    MockOpenAI.defineChat(new ShoppingChat()::translationResponse);
  }

  @Test
  void translationAgent(BpmClient client) {
    var res = client.start().process(TEST_PROCESS.elementName("testTranslationAgent")).execute();
    DemoTestProcessData data = res.data().last();
    assertThat(data.getTranslationResult()).isEqualTo("Hello world");
  }
}
