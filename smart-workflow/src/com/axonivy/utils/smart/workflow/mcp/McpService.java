...
public class McpService {
  ...
  private JsonNode inputSchema(ToolSpecification tool) {
    ...
    try {
      var props = in.putObject("properties");
      tool.parameters().properties().forEach((name, schema) -> {
        props.set(name, toJson(schema));
      });
      return in;
    } catch (Exception ex) {
  ...
  private static JsonNode toJson(JsonSchemaElement schema) {
    var s = JsonSchemaElementJsonUtils.toMap(schema);
    return JsonMapper.shared().valueToTree(s);
  }
